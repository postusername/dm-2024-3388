...
std::string Polynomial::to_string()
{
    ...
    for (std::pair<Natural, Rational> term : this->data)
    {
        if (!first)
        {
            result += ", ";
...
Polynomial operator+(const Polynomial &poly, const Rational &num)
{
...
Polynomial operator*(const Rational &num, const Polynomial &poly)
{
    return poly * num;
}
...
{
    Natural gcf_numerators = this->data.begin()->second.get_numerator().abs();     // начинаем собирать числители для НОД с монома минимальной степени
...
Polynomial operator*(const Polynomial &left, const Polynomial &right)
{
    ...
    for (auto &[deg, val] : right.data)
    {                           // идем по мономам правого многочлена и умножаем их на левый
...
Polynomial operator/(const Polynomial &left, const Polynomial &right)
{
    ...
    {                                                                                    // вычитаем из делимого sub_coefficient пока делимое не станет остатком
...
Polynomial derivative(const Polynomial &X)
{
    ...
    {
        if (deg > Natural(0))
        {                                                        // Пропускаем константный член (степень 0)
