...
ostream& operator<<(ostream& stream, const Natural& num){
    Natural numm = num;
    stream<<numm.to_string(numm);
    return stream;
}
...
string Natural::to_string(Natural num){
...
void Natural::carry_flex(size_t i){
    if (this->data.at(i).carry == 0) return; // Все ок, завершаем
...
void Natural::zero_flex(){
    ...
    for (size_t i = this->data.size() - 1; i > 0; --i){
        if (this->data.at(i) == Digit(0)) j++;
        else break; // Берем все незначащие нули
    }
...
const Natural operator-(const Natural& left, const Natural& right){
    if (left < right) throw invalid_argument("left value must be greater or equal of right value");
    Natural l = left;
    for (size_t i = 0; i < min(left.data.size(), right.data.size()); ++i){
        if (l.data.at(i) < right.data.at(i)){
            for (size_t j = i + 1; j < l.data.size(); ++j){ // Ищем не нулевой разряд (всегда найдется, так как l > r)
...
Natural& Natural::div_nmul(const Natural& n, const Natural k) {
    if (n == Natural(0)) throw invalid_argument("Zero division error");
    Natural i(0), tmp = n;
...
const Natural operator/(const Natural& left, const Natural& right) {
    if (right == Natural(0)) throw invalid_argument("Zero division");
    Natural result(0), current_div_res(0), tmp = left;
    for (long int shift = left.data.size() - right.data.size(); shift >= 0; --shift){ // Делим на right^k, где k = shift
        current_div_res = tmp;
